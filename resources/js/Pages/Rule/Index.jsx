...
function Index({ rules, count, page, termSearch }) {
    ...
    const table = rules.data.map((item, index) => {
        return (
            <tr key={index} className={"border-t transition hover:bg-neutral-100 " + (index % 2 == 0? 'bg-neutral-50': '')}>
                <td className="font-light px-1 py-3"><Link href={route('rules.show', item.id)}>{item.description}</Link></td>
                <td className="font-light px-1 py-3"><Link href={route('rules.show', item.id)}>{item.control}</Link></td>
                <td className="py-3 flex justify-end pr-2 text-neutral-400">
                    <Link href={route('rules.show', item.id)}>
            ...
            <AuthenticatedLayout titleChildren={'Gerenciamento de Regras'} breadcrumbs={[{ label: 'Regras', url: route('rules.index') }]}>
                <div className="flex md:flex-row gap-2 md:gap-4">
                    <Panel className={'inline-flex'}>
                        <Link href={route('rules.create')} className="inline-flex gap-2 items-center justify-between py-2 px-3 rounded-md transition font-light border border-transparent focus:ring bg-blue-500 text-white hover:bg-blue-600 focus:ring-sky-300">
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="h-5 w-5" viewBox="0 0 16 16">
