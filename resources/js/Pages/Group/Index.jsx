...
function Index({ groups, count, page, termSearch, can }) {
    ...
    useEffect(() => {
        const debounce = setTimeout(() => {
            setCurrentPage(1);
            router.get(route(route().current()), {term: term??'', page: currentPage?? 1}, {preserveState: true, replace: true});
        }, 300);
            ...
            <AuthenticatedLayout titleChildren={'Gerenciamento de Páginas'} breadcrumbs={[{ label: 'Páginas', url: route('groups.index') }]}>
                <div className="flex gap-2 md:flex-row md:gap-4">
                    {can.create && <Panel className={'inline-flex'}>
                        <Link href={route('groups.create')} className="inline-flex items-center justify-between gap-2 px-3 py-2 font-light text-white transition bg-blue-500 border border-transparent rounded-md focus:ring-2 hover:bg-blue-600 focus:ring-sky-300">
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="w-5 h-5" viewBox="0 0 16 16">
