...
function Index({ employees, count, page, termSearch, can }) {
    ...
    return (
        <>
            <AuthenticatedLayout
                titleChildren={'Gerenciamento de Mutuários'}
                breadcrumbs={[
                    { label: 'Mutuários', url: route('employees.index') }
                ]}
            >
                <div className="flex gap-2 md:flex-row md:gap-4">
                    {can.create && <Panel className={'inline-flex'}>
                        <Link href={route('employees.create')} className="inline-flex items-center justify-between gap-2 px-3 py-2 font-light text-white transition bg-blue-500 border border-transparent rounded-md focus:ring-2 hover:bg-blue-600 focus:ring-sky-300">
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="w-5 h-5" viewBox="0 0 16 16">
                    ...
                    <table className="w-full table-auto text-neutral-600">
                        <thead>
                            <tr className="border-b border-neutral-300">
                                ...
                                <th className="hidden px-1 pt-3 font-semibold text-left md:table-cell">Autorizadas</th>
                                <th></th>
                            </tr>
                        </thead>
                        <tbody>
